...
const submitReservation = async () => {
  ...
  try {
    // 将日期和时间拼接为时间戳（毫秒），并构建后端需要的 DTO 字段
    const resvDate = reservationForm.date // YYYY-MM-DD
    const resvStart = reservationForm.startTime as string // HH:mm
    const resvEnd = reservationForm.endTime as string // HH:mm

    // 构建 Date 对象并取毫秒，注意本地时区
    const startDateTime = new Date(`${resvDate}T${resvStart}:00`)
    const endDateTime = new Date(`${resvDate}T${resvEnd}:00`)

    // TODO: 当前演示使用本地存储的 userId；你可以根据项目的登录状态来替换
    const userId = Number(localStorage.getItem('userId') || 1)

    const payload = {
      seatId: Number(reservationForm.roomId),
      userId: userId,
      resvDate: resvDate,
      resvStart: resvStart,
      resvEnd: resvEnd,
      resvtimeSignin: startDateTime.getTime(),
      resvtimeSignout: endDateTime.getTime(),
    ...
    // 从后端返回的错误响应中尽可能显示有用信息
    const msg = error?.response?.data?.message || error?.message || '预约失败，请重试'
    ElMessage.error(msg)
  } finally {
