...
const loadViolationStatus = async (userId: string) => {
  try {
    const response = await fetch(`/api/violation/status/${userId}`)
...
const loadViolations = async (userId: string) => {
  loading.value = true
  try {
    const response = await fetch(`/api/violation/user/${userId}`)
    const result = await response.json()
    if (result.code === 200) {
...
const loadAppeals = async (userId: string) => {
  ...
  try {
    const response = await fetch(`/api/violation/appeal/user/${userId}`)
    const result = await response.json()
    if (result.code === 200) {
...
const confirmMakeup = async () => {
  ...
  try {
    const response = await fetch('/api/violation/makeup', {
      method: 'POST',
...
const handleAppeal = async (violation: Violation) => {
  ...
  // 检查是否已申诉
  try {
    const response = await fetch(`/api/violation/appeal/check?userId=${userId}&violationId=${violation.violationId}`)
    const result = await response.json()
...
const submitAppeal = async () => {
  ...
  try {
    const response = await fetch('/api/violation/appeal', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        userId: parseInt(userId),
